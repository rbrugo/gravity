...
void display(std::shared_mutex &, SDLpp::renderer &, entt::registry &, brun::position::value_type const max_radius);

// Generates a function which must be invoked in a thread, which refresh the graphics at a certain rate
inline
auto render_cycle(
    std::shared_mutex & mtx,
    SDLpp::renderer & renderer, entt::registry & registry,
...
) noexcept
{
    ...
    return [&mtx, &renderer, &registry, &max_radius, time_for_frame](std::stop_token token) mutable {
        while (not token.stop_requested()) {
            auto const end = std::chrono::system_clock::now() + time_for_frame;
            display(mtx, renderer, registry, max_radius);
